...
- Mapping validation is implemented in source: provider IDs must exist and selected scheduler service durations must match Better Talk 15/30/45/60-minute durations before mappings are saved. Automated regression coverage now includes valid matches, mismatches, and invalid duration values. Production verification remains pending.
...
- Agent/Admin customer request review queue is implemented in portal source and pending CI/merge/live verification.
